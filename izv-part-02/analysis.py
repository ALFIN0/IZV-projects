    ...
    def get_dataframe(filename: str, zipF: zipfile) -> pd.DataFrame:
        ...
        for k,v in regions.items():
            ...
            with innerZipfile.open(v+'.csv') as csvFile:
                if outputDF is None:
                    outputDF = pd.read_csv(csvFile, delimiter=';', encoding='cp1250', 
                        header=None, names=headers, low_memory=False)
                    outputDF['region'] = k
                else:
                    tmpDF = pd.read_csv(csvFile, delimiter=';', encoding='cp1250', 
                        header=None, names=headers, low_memory=False)
                    tmpDF['region'] = k
...
def plot_visibility(df: pd.DataFrame, fig_location: str = None,
    ...
    visibilityDict = {1: 'VISDAY', 2: 'NOTDAY', 3: 'NOTDAY', 4: 'VISNI', 
        5: 'NOTNI', 6: 'VISNI', 7: 'NOTNI'}
    ...
    ax = axes.flatten()
    graphs = [
        {'ax': ax[0], 'df': b[b.visibility == 'VISNI'], 
            'title': 'Viditelnost v noci - nezhoršená', 'xlabel': None},
        {'ax': ax[1], 'df': b[b.visibility == 'NOTNI'], 
            'title': 'Viditelnost v noci - zhoršená', 'xlabel': None},
        {'ax': ax[2], 'df': b[b.visibility == 'VISDAY'], 
            'title': 'Viditelnost ve dne - nezhoršená', 'xlabel': 'Kraj'},
        {'ax': ax[3], 'df': b[b.visibility == 'NOTDAY'], 
            'title': 'Viditelnost ve dne - zhoršená', 'xlabel': 'Kraj'}
    ]

    for graph in graphs:
        # plot bar graph with region x axis and counts of accidents y axis 
        # with values of current visibility - graph['df']
        sns.barplot(graph['df'], ax=graph['ax'], x='region', y='counts', 
...
def plot_direction(df: pd.DataFrame, fig_location: str = None,
    ...
    for graph in graphs:
        # plot bar graph with date x axis and counts of accidents y axis and hue as 
        # precipitation with values of current region - graph['df']
        sns.barplot(graph['df'], ax=graph['ax'], x='date', y='counts', 
            hue='precipitation', palette=['tab:blue', 'tab:orange', 'tab:green'])
        graph['ax'].set_title(graph['title'])
...
def plot_consequences(df: pd.DataFrame, fig_location: str = None,
                    show_figure: bool = False):
    """Plot graph of consequences of accidents in chosen regions in time.
    ...
    # create pivot table with region-month indexes based on collumn 'Následky' - consequences
    table = pd.pivot_table(newDF, columns=['Následky'], values='p1', 
    index=['region', 'month'], aggfunc='count')
    ...
    plot.figure.subplots_adjust(right=0.88, top=0.9, wspace=0.1, hspace=0.2)
